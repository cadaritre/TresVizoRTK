...
const pageNames = {overview: 'Resumen', connections: 'Conexiones', settings: 'Configuración', diagnostics: 'Diagnóstico'};
let accessKey = '';
let config = null;
...
async function api(path, method = 'GET', body) {
  ...
  try {
    ...
    const response = await fetch(path, {method, headers, body: body === undefined ? undefined : JSON.stringify(body), signal: controller.signal, cache: 'no-store'});
    let data;
    try { data = await response.json(); } catch { throw new Error('La respuesta del instrumento no es válida.'); }
    ...
    if (error.name === 'AbortError') throw new Error('El instrumento no respondió a tiempo.');
    throw error;
...
function connected(ok) {
  $('connection-pill').className = `connection-pill ${ok ? 'online' : 'offline'}`;
  text('connection-text', ok ? 'Instrumento conectado' : (awaitingRestart ? 'Reiniciando…' : 'Sin conexión'));
  $('offline-notice').hidden = ok || awaitingRestart || $('login-dialog').open;
  $('save-settings').disabled = saving || !ok || !config || !config.persistence_ready;
  $('restart-button').disabled = !ok;
  $('export-diagnostics').disabled = !ok;
  if (!ok) {
    latestStatus = null;
    document.querySelectorAll('[data-live]').forEach((el) => { el.textContent = '—'; });
...
function renderStatus(data) {
  if (data.api_version !== 1 || typeof data.uptime_ms !== 'number' || !data.wifi) throw new Error('Versión de protocolo no compatible.');
  latestStatus = data;
  refreshMs = [1000, 2000, 5000].includes(data.refresh_ms) ? data.refresh_ms : 2000;
  connected(true);
  ...
  text('ap-summary', data.wifi.ap_ready ? `Panel en ${data.wifi.ap_ip}` : 'No se pudo iniciar la red');
  ...
  const states = {connected: 'Conectado', connecting: 'Intentando conectar', not_configured: 'Sin configurar'};
  ...
  text('station-rssi', data.wifi.rssi_dbm === null ? '—' : `${data.wifi.rssi_dbm} dBm`);
  text('diag-chip', `${data.chip} · rev. ${data.chip_revision}`);
  text('diag-cpu', `${data.cpu_mhz} MHz`);
  text('diag-flash', `${data.flash_bytes / 1048576} MB`);
  text('diag-psram', data.psram_enabled_bytes ? kib(data.psram_enabled_bytes) : 'Desactivada en esta versión');
  text('diag-min-heap', kib(data.min_free_heap_bytes));
  const reasons = {1: 'Encendido', 3: 'Reinicio por software', 4: 'Excepción', 5: 'Watchdog de interrupción', 6: 'Watchdog de tarea', 9: 'Caída de tensión'};
  text('diag-reset', reasons[data.reset_reason_code] || `Código ${data.reset_reason_code}`);
  text('diag-config', data.config_ready ? 'Disponible' : 'Requiere atención');
  text('last-updated', `Actualizado ${new Date().toLocaleTimeString('es-MX', {hour12: false})}`);
}
async function poll() {
  ...
  finally { polling = false; timer = setTimeout(poll, refreshMs); }
}
function fillConfig(value) {
  ...
  text('password-help', value.wifi_password_saved ? 'Ya hay una contraseña guardada. Deja vacío para conservarla si usas la misma red.' : 'Entre 8 y 63 caracteres. La contraseña no se devuelve al navegador.');
  $('save-settings').disabled = saving || !latestStatus || !value.persistence_ready;
  dirty = false;
}
async function loadConfig() {
  try {
    fillConfig(await api('/api/config'));
    message(config.stored_config_valid ? 'Ajustes cargados del instrumento.' : 'La configuración guardada no era válida. Revisa y guarda los ajustes.', !config.stored_config_valid);
  } catch (error) { message(error.message, true); }
}
$('settings-form').addEventListener('input', () => { dirty = true; message('Tienes cambios sin guardar.'); });
...
$('settings-form').addEventListener('submit', async (event) => {
  ...
  const body = {revision: config.revision, device_name: $('device-name-input').value, refresh_ms: Number($('refresh-input').value), wifi_ssid: $('wifi-ssid-input').value, wifi_password: $('wifi-password-input').value, forget_wifi: $('forget-wifi').checked};
  try {
    fillConfig(await api('/api/config', 'PUT', body));
    message('Ajustes guardados en el instrumento.');
    poll();
  } catch (error) { message(error.message, true); }
  finally { saving = false; $('reload-settings').disabled = false; $('save-settings').disabled = !latestStatus || !config.persistence_ready; }
});
...
$('login-form').addEventListener('submit', async (event) => {
  ...
  try {
    ...
    renderStatus(status);
    await loadConfig();
  } catch (error) { accessKey = ''; text('login-message', error.message); }
});
...
$('confirm-restart').addEventListener('click', async () => {
  ...
  try {
    ...
    dirty = false;
    connected(false);
    setTimeout(() => { awaitingRestart = false; poll(); }, 4000);
  } catch (error) { awaitingRestart = false; $('restart-dialog').close(); message(error.message, true); goTo('settings'); poll(); }
