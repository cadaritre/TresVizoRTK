...
for z in (96,139):
    ...
    # M3 avellanado: asiento de 90 grados para cabeza <= Ø5.6.
    y=outer_radius(z)
    panel=panel.cut(Part.makeCone(1.7,2.9,1.2,V(0,y-1.2,z),V(0,1,0)))
...
for i,(x,y) in enumerate(holes):
    screw=Part.makeCylinder(.95,5,V(x,y,board_z+1.6),V(0,0,-1))
    screw=screw.fuse(Part.makeCylinder(1.8,1.3,V(x,y,board_z+1.6)))
    add('USBScrew'+str(i+1),'M2 × 5 · montaje USB',screw,(145,155,165),'reference')
for i,x in enumerate((-9,9)):
    screw=cyl_y(x,bz,back-2.5,.95,8).fuse(cyl_y(x,bz,back-3.9,1.8,1.4))
...
excluded={'Plastic','Hardware','References','ServiceCover','USBReference','LatchReserve','IMUAlignmentGauge'}
for src in original.Objects:
    ...
    if src.Name=='ElectronicsTray':
        # Quitar sólo la repisa antigua del USB que invade el nuevo montaje.
        shape=shape.cut(box(-11.6,12.0,124.0,23.2,17.0,10.8))
        # Retirar la cuna provisional del antiguo biestable, ya cancelada.
        shape=shape.cut(box(-12.3,24.4,103,24.6,4.7,14))
    ...
    add('A5_'+src.Name,src.Label+(' · modificación propuesta' if src.Name in ('MainShell','ElectronicsTray') else ' · A5'),shape,color,'context',note='Copia embebida de A5; maestro conservado.')
...
doc.saveAs(str(ROOT/'TresVizo-panel-modules.FCStd'))
Part.export([objects[n] for n in printed+references],str(ROOT/'panel-assembly.step'))
Part.export([objects['ModulePanel'],objects['A5_MainShell'],objects['A5_ElectronicsTray']],str(ROOT/'case-integration.step'))
for n in printed+['A5_MainShell','A5_ElectronicsTray']:
    mesh=MeshPart.meshFromShape(Shape=objects[n].Shape,LinearDeflection=.06,AngularDeflection=.12,Relative=False)
    mesh.write(str(ROOT/'stl'/(n+'.stl')))
...
MeshPart.meshFromShape(Shape=coupon,LinearDeflection=.04,AngularDeflection=.1,Relative=False).write(str(ROOT/'stl'/'fit-coupon.stl'))
...
(ROOT/'render-meshes.json').write_text(json.dumps(meshes,separators=(',',':')))
