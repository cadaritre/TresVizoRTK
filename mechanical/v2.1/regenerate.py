"""Regenera V2.1 completa. Portable: localiza FreeCAD en vez de tener su ruta
...
# Tornilleria primero: capacity_check.py usa los largos que elige.
print('\n--- bom.py ---')
subprocess.run([sys.executable, str(ROOT / 'bom.py')], check=True, stdout=subprocess.DEVNULL)

# El python que trae FreeCAD en macOS no encuentra sus propios modulos sin
# PYTHONPATH apuntando a su carpeta lib.
env = dict(os.environ)
...
for script in ('build_v2_1.py', 'export_v2_1.py', 'capacity_check.py'):
    print(f'\n--- {script} ---')
    subprocess.run([str(freecad), str(ROOT / script)], check=True, env=env)
...
cap = json.loads((ROOT / 'generated' / 'capacity.json').read_text(encoding='utf-8'))
listo = listo and cap['cabe_todo']
print('un solido por pieza :', solidos_ok)
print('cabe y seguros      :', cap['cabe_todo'], cap['fallos'] or '')
